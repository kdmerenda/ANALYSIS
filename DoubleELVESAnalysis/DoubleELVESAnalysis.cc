...
DoubleELVESAnalysis::Run(evt::Event& event)
{
  ...
  for (AugerEvent::EyeIterator eyeIter = rawEvent.EyesBegin();
	 ...
	 g1.GetParameter(2) > 200 || g2.GetParameter(2) > 200)
	{
     ...
     if (FirstPeak.size() != 0){
       cout << FirstPeak[0].A << " " << FirstPeak[FirstPeak.size()-1].A << endl;
       cout << FirstPeak[0].B << " " << FirstPeak[FirstPeak.size()-1].B << endl;
       cout << FirstPeak[0].C << " " << FirstPeak[FirstPeak.size()-1].C << endl << endl;
     }
